...
class MOEAD_P:
    def __init__(self, toolbox, cxpb, mutpb, m, lambda_, neighbor_size=10, criteria=200):
    ...
    def execute(self):
        ...
        while self.criteria > 0:
            ...
            for i in range(self.population_size):
                ...
                # 变异操作
                offspring = self.mutate(self.population[i])
                ...
                if offspring:
                    ...
                    if self.get_fitness(offspring, self.weights[i]) < self.get_fitness(self.population[i], self.weights[i]):
                        self.population[i] = deepcopy(offspring)
                    self.update_ideal_point(self.population[i])             # 更新参考点
                    self.update_neighbor(self.B[i], self.population[i])     # 更新邻居
                    self.update_ep(self.population[i])                      # 更新外部种群
                elif flag:
                    self.update_ideal_point(self.population[i])             # 更新参考点
                    self.update_neighbor(self.B[i], self.population[i])     # 更新邻居
                    self.update_ep(self.population[i])                      # 更新外部种群
    ...
    def init_sort(self):
        population = []                                                 # 初始化新种群
        for i in range(self.population_size):
            fitness = [self.get_fitness(self.population[j],
                                        self.weights[i])
                       for j in range(i, self.population_size)]         # 计算出第i权重向量与未被选择的个体间的适应度
            m = min(fitness)                                            # 找到最小的那个
            n = fitness.index(m)                                        # 得到ID
            population.append(self.population[n])                       # 将该个体添加到种群中
    ...
    def get_fitness(self, individual, lambda_):
        ...
        # fitness = sum([values[i] * lambda_[i] for i in range(self.m)])
        return fitness
    ...
    def update_neighbor(self, neighbors, individual):
        ...
        for i in range(self.neighbor_size):
            ...
            if self.get_fitness(individual, weight) < self.get_fitness(self.population[neighbors[i]], weight) and n <= 1:
